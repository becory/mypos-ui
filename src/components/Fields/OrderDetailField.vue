    ...
    <el-table v-if="data.length>0" :data="data" :show-header="false">
      ...
      <el-table-column>
        <template slot-scope="scope">
          ...
          <el-form-item>
            <el-select v-model="scope.row.cook">
              <el-option v-for="(item,idx) in cookOptions" :key="idx" :value="item.value" :label="item.label" />
            </el-select>
...
import { getCookList } from '@/api/api'
...
export default {
  ...
  methods: {
    getSelectionOptions() {
      ...
      this.getCookSelection().then((res) => (this.cookOptions = res))
    },
    onAdd() {
      const result = {
    ...
    getCookSelection() {
      return getCookList('?selection=name')
        .then((res) => {
